...
def setup_database():
    ...
    if conn is None:
        logging.critical("Could not establish a database connection. The bot cannot continue.")
        # In a real scenario, you might want to exit or retry.
        return

    with conn:
        with conn.cursor() as cur:
            # Using BIGINT for user_id to be safe with Telegram's 64-bit IDs
...
# Call the setup function when the bot starts.
setup_database()
...
def add_user(user_id, name, referrer=None):
    ...
    with conn.cursor() as cur:
        ...
        cur.execute("INSERT INTO users (user_id, name, credit_points, referrer, last_update) VALUES (%s, %s, 5, %s, %s)", (user_id, name, referrer, now))
        conn.commit()
    conn.close()
    if referrer:
...
def update_points(user_id, amount, point_type):
    ...
    with conn.cursor() as cur:
        ...
        query = f"UPDATE users SET {column_name} = {column_name} + %s, last_update = %s WHERE user_id = %s"
...
def log_transaction(user_id, point_type, amount, description):
    ...
    with conn.cursor() as cur:
        now = datetime.datetime.now(datetime.timezone.utc)
        cur.execute("INSERT INTO transactions (user_id, point_type, amount, timestamp, description) VALUES (%s, %s, %s, %s, %s)",
...
def get_deposit_history(user_id, limit=10):
    ...
    history = []
    with conn.cursor() as cur:
        cur.execute("SELECT point_type, amount, timestamp FROM transactions WHERE user_id=%s AND amount > 0 ORDER BY timestamp DESC LIMIT %s", (user_id, limit))
...
def claim_bonus(user_id):
    ...
    # Ensure the date from the DB (if it exists) is also a date object for comparison
    last_bonus_date = user.get('last_bonus_date')
    if isinstance(last_bonus_date, str):
        last_bonus_date = datetime.date.fromisoformat(last_bonus_date)

    if user is None or last_bonus_date == today:
        return False

    conn = get_db_connection()
    if not conn: return False
    with conn.cursor() as cur:
        cur.execute("UPDATE users SET credit_points = credit_points + 2, last_bonus_date=%s WHERE user_id=%s", (today, user_id))
        conn.commit()
    conn.close()
    return True
...
def update_total_requests_in_db(user_id):
    conn = get_db_connection()
    if not conn: return
    with conn.cursor() as cur:
        cur.execute("UPDATE users SET total_requests = total_requests + 1 WHERE user_id = %s", (user_id,))
        conn.commit()
    conn.close()
...
def prompt_channel_join(chat_id):
    text = (
        f"""👋 HEY DEAR, {bot.get_chat(chat_id).first_name}
☢️ **Note**: *MUST JOIN OUR CHANNEL TO USE THE BOT.*
➡️ CLICK ON ✅ **JOINED** AFTER JOINING THE CHANNEL"""
    )
    markup = types.InlineKeyboardMarkup(row_width=2)
...
def deposit_history_handler(call):
    ...
    for p_type, amount, ts in history:
        ...
        # The timestamp is already timezone-aware from the DB
...
def top_referrers_handler(call):
    ...
    for rank, (referrer_id, count) in enumerate(top_leaders, 1):
        # It's more efficient to get all names at once, but for a top 5 list, this is fine.
    ...
    while time.time() < end_time and not stop_requested:
        try:
            ...
            if response.status_code == 200:
                ...
                update_total_requests_in_db(chat_id)
            else:
                failed_count += 1
...
if __name__ == "__main__":
    if DATABASE_URL == "YOUR_POSTGRESQL_DATABASE_URL_HERE":
        logging.critical("The DATABASE_URL variable is not configured. The bot cannot start.")
    else:
